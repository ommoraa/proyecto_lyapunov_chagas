...
def load_ajuste():
    ...
    # Detectar columna de modelo/simulación
...
def load_lyapunov():
    """
    Carga el resultado del cálculo de Lyapunov desde
    models/lyapunov_valledupar.json, usando las claves:
    ...
    T_total = float(data.get("T_total_days", 0.0))
    dt_block = float(data.get("dt_block_days", 0.0))

    if T_total > 0.0 and dt_block > 0.0:
        n_blocks = int(T_total / dt_block)
    else:
        n_blocks = 0

    return {
        "municipio": data.get("municipio", "Valledupar"),
        "lambda_max": lambda_max,
        "T_total_days": T_total,
        "dt_block_days": dt_block,
        "n_blocks": n_blocks,
    }
...
if (lyap_info is not None) and (lyap_info["n_blocks"] > 0):
    st.markdown(
        f"""
- Municipio: **{lyap_info["municipio"]}**  
- Exponente de Lyapunov máximo (λₘₐₓ): **{lyap_info["lambda_max"]:.6f}**  
- Horizonte total de integración: **{lyap_info["T_total_days"]:.0f} días**  
- Tamaño de bloque utilizado: **{lyap_info["dt_block_days"]:.0f} días**  
- Número de bloques utilizados: **{lyap_info["n_blocks"]}**
"""
    )
    st.markdown(
        "- Un valor de λₘₐₓ positivo pero cercano a cero indica una dinámica "
        "predominantemente estable, con sensibilidad al estado inicial muy "
        "débil en el horizonte de tiempo considerado."
    )
else:
    st.info(
        "No se encontraron resultados de Lyapunov. "
        "Ejecute la etapa `compute_lyapunov` del pipeline para generar "
...
ax.axhline(
    ...
    color="red",
    linestyle="--",
    label=f"Umbral alta incidencia ({percentil:.2f})",
)
ax.set_xlabel("Fecha")
